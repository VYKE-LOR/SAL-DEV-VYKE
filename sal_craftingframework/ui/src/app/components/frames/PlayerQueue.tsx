...
export const PlayerQueue: React.FC<PlayerQueueProps> = ({ frameId, onClose, onBack }) => {
  const { state, actions } = useNui();
  const queue = useMemo(() => {
    const snapshotQueue = state.snapshot.queue;
    if (Array.isArray(snapshotQueue) && snapshotQueue.length > 0) {
  ...
  return (
    <div id={frameId} className="w-full h-full bg-cw-bg text-cw-text-primary overflow-hidden flex flex-col font-sans">
       <div id="cw_queue_root" className="flex flex-col h-full">
         <PlayerTopbar title="CRAFTING QUEUE" subtitle="Current Production" onClose={onClose} onOpenBlueprints={onBack} />
                   ...
                   <Card key={item.id} id={`row_queue__${item.id}`} className="p-4 flex items-center gap-6 group hover:border-cw-primary/30 transition-all">
                       ...
                       <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
                           ...
                           <div className="col-span-4 flex justify-end gap-2">
                               {item.status === 'ready' ? (
                                   <Button id="btn_queue_claim" variant="primary" className="shadow-lg shadow-cw-success/20 bg-cw-success hover:bg-cw-success/90 text-cw-bg font-bold" onClick={() => actions.queueClaim(item.id)}>
                                       Claim Item
                                   </Button>
                               ) : (
                                   <>
                                     <Button id="btn_queue_details" variant="minimal" size="sm">Details</Button>
                                     <Button id="btn_queue_cancel" variant="danger" size="icon" className="w-9 h-9 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => actions.queueCancel(item.id)}>
                                         <X className="w-4 h-4" />
                                     </Button>
                                   </>
                               )}
