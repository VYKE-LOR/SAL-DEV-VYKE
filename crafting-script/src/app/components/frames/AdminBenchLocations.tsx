             ...
             <div id="ad_content" className="flex-1 flex p-8 gap-8 overflow-hidden">
                 ...
                 <div className="flex-1 flex flex-col gap-6">
                     ...
                     <Card id="lst_benchLocations" className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-cw-surface-1/30">
                        {[1, 2, 3].map(id => (
                           <div key={id} id={`row_benchLocation__${id}`} className="bg-cw-surface-1 p-4 rounded-xl border border-cw-border flex items-center justify-between hover:border-cw-primary/50 transition-colors">
                              <div className="flex flex-col gap-1">
                                  <div className="flex items-center gap-3">
                                      <span id="txt_location_benchType" className="font-bold text-cw-text-primary">workbench</span>
                                      <Badge id="badge_location_portable" variant={id === 1 ? "success" : "secondary"}>{id === 1 ? "Portable" : "Static"}</Badge>
                                  </div>
                                  <div className="flex gap-4 text-xs text-cw-text-secondary font-mono">
                                     <span id="txt_location_coords">vector3(123.45, -45.12, 30.0)</span>
                                     <span id="txt_location_heading">h: 120.5</span>
                                  </div>
                              </div>
