...
export const PlayerCrafting: React.FC<PlayerCraftingProps> = ({ frameId }) => {
    ...
    <div id="cmp_cw_profile_panel" className="flex flex-col gap-6 p-8 max-w-4xl mx-auto w-full">
      <Card id="cw_profile_level_card" className="p-6 flex flex-col gap-4">
        <div className="flex justify-between items-end">
            ...
            <span id="txt_profile_xp_value" className="text-cw-text-primary font-mono">1200 / 2000 XP</span>
        </div>
        <div className="flex items-center gap-4">
             <div className="bg-cw-primary/20 rounded-lg p-3">
                <span id="txt_profile_level_value" className="text-4xl font-bold text-cw-primary">12</span>
             </div>
                  ...
                  {CATEGORIES.map(cat => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                         ...
                         <span id="txt_category_name" className="text-sm font-medium text-cw-text-secondary group-hover:text-cw-text-primary">{cat.name}</span>
                      </div>
                      <Badge id="badge_category_count" variant="secondary" className="bg-cw-bg">{cat.count}</Badge>
                    </div>
                  ))}
               </div>
            </div>
                 ...
                 >
                    ...
                    <div id="cw_recipe_meta" className="flex-1 min-w-0 flex flex-col gap-1">
                       ...
                       <div id="cw_recipe_chips" className="flex gap-2 mt-1">
                          <Badge id="chip_recipe_category" variant="outline" className="text-[10px] py-0 h-5">{recipe.category}</Badge>
                          {recipe.level > 1 && <Badge id="badge_level_req" variant="secondary" className="text-[10px] py-0 h-5">LVL {recipe.level}</Badge>}
                          {recipe.bpRequired && <Badge id="badge_blueprint_req" variant="warning" className="text-[10px] py-0 h-5">BP REQUIRED</Badge>}
                       </div>
                    </div>
                    ...
                    <div id="cw_recipe_status" className="w-32 flex flex-col justify-between items-end pl-2 border-l border-cw-border/50">
                        <div className="w-full text-right">
                           ...
                           <div className="h-1.5 w-full bg-cw-bg rounded-full overflow-hidden">
                              <div id="prg_materials" className="h-full bg-cw-success transition-all" style={{ width: `${recipe.materialsProgress}%` }}></div>
                           </div>
                        </div>
                  ...
                  <div id="cw_detail_header" className="p-6 border-b border-cw-border bg-gradient-to-b from-cw-surface-2/50 to-transparent">
                     <div className="flex gap-5">
                        ...
                        <div className="flex flex-col gap-2">
                           <h2 id="txt_detail_recipe_name" className="text-2xl font-bold text-cw-text-primary">Assault Rifle</h2>
                           <p id="txt_detail_recipe_desc" className="text-sm text-cw-text-secondary leading-relaxed">Standard issue automatic rifle. Reliable and accurate for mid-range combat.</p>
                        </div>
                     </div>
                     <div id="cw_detail_badges" className="flex flex-wrap gap-2 mt-4">
                        <Badge id="badge_detail_level_req" variant="secondary">LEVEL 3 REQUIRED</Badge>
                        <Badge id="badge_detail_blueprint_req" variant="warning">BLUEPRINT REQUIRED</Badge>
                        <Badge id="badge_detail_time" variant="outline" className="gap-1"><Clock className="w-3 h-3"/> 45s</Badge>
                        <Badge id="badge_detail_xp" variant="primary" className="gap-1"><Zap className="w-3 h-3"/> +120 XP</Badge>
                     </div>
                  </div>
                  ...
                  <div className="p-6 flex flex-col gap-6">
                     ...
                     <Card id="cw_ingredients_card" className="p-0 overflow-hidden">
                        ...
                        <div id="lst_ingredients" className="divide-y divide-cw-border/50">
                           {INGREDIENTS.map((ing) => (
                              <div key={ing.id} id={`row_ingredient__${ing.id}`} className="flex items-center justify-between p-3 hover:bg-cw-surface-2/30">
                                 <div className="flex items-center gap-3">
                                    <div id="img_ingredient_icon" className="w-8 h-8 rounded bg-cw-bg border border-cw-border" />
                                    <div className="flex flex-col">
                                       <span id="txt_ingredient_label" className="text-sm font-medium text-cw-text-primary">{ing.name}</span>
                                       <span className="text-[10px] text-cw-text-muted">
                                          <span id="txt_ingredient_required">Required: {ing.required}</span> • <span id="txt_ingredient_have" className={ing.have < ing.required ? "text-cw-error" : "text-cw-success"}>Have: {ing.have}</span>
                                       </span>
                                    </div>
                                 </div>
                     ...
                     <Card id="cw_outputs_card" className="p-0 overflow-hidden">
                        ...
                        <div id="lst_outputs" className="p-3">
                           <div id="row_output__assault_rifle" className="flex items-center gap-3">
                               <div id="img_output_icon" className="w-10 h-10 rounded-lg bg-cw-bg border border-cw-border" />
                               <div className="flex flex-col">
                                  <span id="txt_output_label" className="font-bold text-cw-text-primary">Assault Rifle</span>
                                  <span id="txt_output_amount" className="text-xs text-cw-text-secondary">x1</span>
                               </div>
                           </div>
                        </div>
                     </Card>
                     ...
                     <Card id="cw_craft_controls_card" className="mt-auto p-4 bg-cw-surface-2 border-cw-primary/30 shadow-[0_0_20px_rgba(122,47,251,0.1)]">
                        <div className="flex justify-between items-center mb-4">
                           ...
                           <span id="txt_craft_time" className="text-sm font-mono text-cw-text-secondary">Time: 45s</span>
                        </div>
                        ...
                        <div id="cw_craft_buttons" className="grid grid-cols-2 gap-3 mb-2">
                           <Button id="btn_craft_queue" variant="secondary" className="w-full">Add to Queue</Button>
                           <Button id="btn_craft_start" variant="primary" className="w-full shadow-lg shadow-cw-primary/20">Craft Now</Button>
                        </div>
